    ...
    <div className="fixed inset-0 z-40 bg-black/30 flex items-center justify-center p-4 overflow-y-auto" onClick={onCancel}>
      <div onClick={(e) => e.stopPropagation()} className="w-full max-w-2xl rounded-2xl p-5 border my-8" style={{ background: 'var(--bg)', borderColor: 'var(--rule)' }}>
        ...
        <div className="grid sm:grid-cols-2 gap-3 text-[13px]">
          ...
          <Field label="نص مساعد (ع)"><Input value={form.help_ar} onChange={(v) => setForm({ ...form, help_ar: v })} /></Field>
          <Field label="نص مساعد (en)"><Input value={form.help_en} onChange={(v) => setForm({ ...form, help_en: v })} dir="ltr" /></Field>
          <Field label="placeholder (ع)"><Input value={form.placeholder_ar} onChange={(v) => setForm({ ...form, placeholder_ar: v })} /></Field>
          <Field label="placeholder (en)"><Input value={form.placeholder_en} onChange={(v) => setForm({ ...form, placeholder_en: v })} dir="ltr" /></Field>
          <Field label="الترتيب"><Input value={String(form.position)} onChange={(v) => setForm({ ...form, position: Number(v) || 0 })} dir="ltr" /></Field>
          <Field label="دور الحقل (semantic_role)">
            <select value={form.semantic_role ?? ''} onChange={(e) => setForm({ ...form, semantic_role: e.target.value || null })}
              className="w-full px-3 py-2 rounded-lg border bg-transparent text-[13.5px]"
              style={{ borderColor: 'var(--rule)', color: 'var(--fg)' }}>
            ...
            <div className="space-y-2">
              {form.options.map((opt, i) => (
                <div key={i} className="grid grid-cols-[1fr_1fr_auto] gap-2">
                  <Input value={opt.ar} onChange={(v) => {
                    const next = form.options.slice(); next[i] = { ...next[i], ar: v }; setForm({ ...form, options: next });
                  }} />
                  <Input value={opt.en} onChange={(v) => {
                    const next = form.options.slice(); next[i] = { ...next[i], en: v }; setForm({ ...form, options: next });
                  }} dir="ltr" />
                  <button onClick={() => setForm({ ...form, options: form.options.filter((_, j) => j !== i) })} className="text-[13px] px-2" style={{ color: 'var(--danger)' }}>حذف</button>
                </div>
              ))}
              <button
